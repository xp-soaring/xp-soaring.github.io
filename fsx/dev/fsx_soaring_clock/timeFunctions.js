 ...
 if (city != "< Choose City >") {
  for (var i=0; i < cityArray.length; i++) {
   if (city == cityArray[i].cityAcronym) {
    ...
    document.getElementById(id+"FSX").value = displayFSXtime(cityArray[i]);
    if (document.all) {
  ...
  else {document.theForm.elements[id].value = "";}
}

function setCity(name, cityArray) {
  //setFSXtime(name, cityArray);
  for (var i=0; i < cityArray.length; i++) {
   if (name == cityArray[i].cityName) {
    document.getElementById(name).value = displayDate(cityArray[i]);
    if (document.all) {
     window.sstchur.web.js.xb.setTimeout(setCity, 1000, name, cityArray);
    }
    else { window.setTimeout(setCity, 1000, name, cityArray); }
   }
  }
}

function setFSXtime(name, cityArray) {
  for (var i=0; i < cityArray.length; i++) {
   if (name == cityArray[i].cityName) {
    document.getElementById(name+"FSX").value = displayFSXtime(cityArray[i]);
    if (document.all) {
     window.sstchur.web.js.xb.setTimeout(setFSXtime, 1000, name, cityArray);
    }
    else { window.setTimeout(setFSXtime, 1000, name, cityArray); }
   }
  }
}
...
function displayDate(theCity) {
 ...
 var dispDT = formatNum(hourDisp.hour);
 dispDT += ":";
...
function displayFSXtime(theCity) {
 localDate = new Date();
 localTime = localDate.getTime();
 UTCoffset = localDate.getTimezoneOffset();
 UTCTime = localTime + (UTCoffset * 60 * 1000);
 ...
 var hour = hourDisp.hour;
 while (hour<11 || hour>=19) {
     hour = hour - 8;
     if (hour<0) hour = hour + 24;
 }
 var dispDT = formatNum(hour);
 dispDT += ":";
 dispDT += formatNum(theDate.getMinutes());
 dispDT += ":";
 dispDT += formatNum(theDate.getSeconds());
...
function formatGMT() {
   var now = new Date();
   var hour = now.getHours()+UTCoffset;
   if (hour<0) hour = hour + 24;
   var min = now.getMinutes();
   var sec = now.getSeconds();
   ...
   if (sec <= 9) { sec = "0" + sec; }
   if (hour < 10) { hour = "0" + hour; }
  document.theForm.theGMT.value = hour + ':' + min + ':' + sec;
  setTimeout("formatGMT()", 1000);
}
//-->